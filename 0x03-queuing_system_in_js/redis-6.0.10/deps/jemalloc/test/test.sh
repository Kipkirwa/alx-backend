...
for t in $@; do
  ...
  if [ -e "${t}.sh" ] ; then
    ...
    . ${t}.sh && \
    export_malloc_conf && \
    $JEMALLOC_TEST_PREFIX ${t} /alx-backend/0x03-queuing_system_in_js/redis-6.0.10/deps/jemalloc/ /alx-backend/0x03-queuing_system_in_js/redis-6.0.10/deps/jemalloc/
  else
    export MALLOC_CONF= && \
    export_malloc_conf && \
    $JEMALLOC_TEST_PREFIX ${t} /alx-backend/0x03-queuing_system_in_js/redis-6.0.10/deps/jemalloc/ /alx-backend/0x03-queuing_system_in_js/redis-6.0.10/deps/jemalloc/
